...
export const ProfessionalSchema = z.object({ id: z.string().optional(), name: z.string().trim().min(3).max(120), specialtyId: z.string().min(1, "Selecione uma especialidade."), registration: z.string().trim().min(3).max(80), phone: z.string().trim().min(8).max(24), email: z.string().trim().email("Informe um e-mail válido.") });
